...
def _natural_sorted(names: list[str]) -> list[str]:
    """Sort frame_00, frame_01 ... in human order."""
    return sorted(names)
...
class Animation:
    ...
    def update(self, dt: float, speed: float = 1.0) -> None:
        ...
        # Use a while-loop so very large dt (e.g. after a stall) still resolves
        # to the correct frame instead of skipping the accumulator.
