...
class Character:
    ...
    def __init__(self, folder: str, scale: float = 1.0):
        ...
        # How many fully-transparent pixel rows are at the bottom of the idle
        # sprite. Used by the widget to nudge the window down so the character's
        # feet (not the transparent padding) sit on the taskbar surface.
        self.floor_offset = self._compute_floor_offset(self.animations["idle"])
    ...
    @staticmethod
    def _compute_floor_offset(anim) -> int:
        """Number of fully-transparent pixel rows at the bottom of the first frame."""
        if not anim or not anim._frames:
            return 0
        from PyQt5.QtGui import QImage
        img = anim._frames[0].toImage().convertToFormat(QImage.Format_ARGB32)
        w, h = img.width(), img.height()
        for row in range(h - 1, -1, -1):
            for col in range(w):
                if img.pixelColor(col, row).alpha() > 0:
                    return h - 1 - row
        return 0
